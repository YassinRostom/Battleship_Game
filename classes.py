...
class Ship:
    def __init__(self, bow, horizontal, length, hit):
        ...
        self._length - int
        length of the ship in [1;5] inclusive
        ...
        *
        ...
        """
        self.bow = bow                      # (int, int) - coordinate of ship's
        self.horizontal = horizontal        # bool - True = Horizontonal , False = Vertical
        self._length = length               # Int  - length of ship 1, 2 , 3 , 4 cells
        self._hit = hit                     # lst - Check doc string - True = Injured
    ...
    def shoot_at(self, coordinate):
        ...
        """
        if self.horizontal:                                 # Horizontal Ship
    ...
    def is_hit(self, coordinate):
        ...
        a right coordinate that is in this ship
        """
        if self.horizontal:                                 # Checks if coordinate cell in ship was hit or not
    ...
    def is_dead(self):
        ...
        """
        return self._hit.count(False) == 0      # Check hit list, if all True (No False) ship is dead
...
class Field:
    def __init__(self):
        ...
        visited = set()
        for cell in field:                  # iterate over dic index --> (1,1), (1,2)
            if cell not in visited:
                length, coordinates = ship_size(field, cell)
                if length == 0:             # Means no ship in this cell
                    self._field[cell] = False
                else:                       # Means there is a ship in this cell
                    visited = visited.union(coordinates)                        # Return all elements in the set coordinates
                    horizontal = (max(coordinates)[0] == min(coordinates)[0])   # Logic to determine Horizontal or Vertical
    ...
    def _get_field(self, displayship):
        ...
        '''
        """
        text_field = [[' ']*10 for i in range(10)]      # Generate 10x10 Empty Cells Field
        for line in range(1, 11):       # Column
            for row in range(1, 11):    # row
                cell = (line, row)
                if isinstance(self._field[cell], Ship):     # Check if this cell has a ship instance or not --> Check if "self._field[cell]" is an instance of "cell"
                ...
                else:                                       # cell has no ship
                    if self.is_hit(cell):                   # cell has no ship and is hit
    ...
    def is_hit(self, coordinate):
        ...
        in [(1, 1); (10, 10)] inclusive
        """
        if isinstance(self._field[coordinate], Ship):       # cell has a ship instance
    ...
    def shoot_at(self, coordinate):
        ...
        returns False
        """
        if isinstance(self._field[coordinate], Ship):   # Cordinate was not hit
            self.cells_left -= 1
            self._field[coordinate].shoot_at(coordinate)    # Set the cell to True to indicate it has been Hit
            return True
        else:   # Cordinate is hit before
